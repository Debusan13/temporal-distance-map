...
for warp in warpCoords:
    ...
    if voronoi:
        ...
        interpType= 'linear'
        interpFunctionDist = Rbf(list_new_x,list_new_y,list_old_dist, function = interpType)
    ...
    for i in range(0, mapResolution):
        ...
        for j in range(0, mapResolution):
            ...
            if voronoi:
                distances, ndx = tree.query([p,o], k=1)
                unwarped_x = list_old_x[ndx]
                unwarped_y = list_old_y[ndx]
            else:
                mapDist = interpFunctionDist(p, o)
                normalized = l2norm(p-0.5, o-0.5)
                unwarped_x = normalized[0]*mapDist+0.5
                unwarped_y = normalized[1]*mapDist+0.5
            
            if dist(p, o, 0.5, 0.5) < 0.005:
                color = [255,0,0,255]
            else:
            ...
            if (dist(p, o, 0.5, 0.5) % (oneMinuteDistance*ringMinutes)) < 0.002:
